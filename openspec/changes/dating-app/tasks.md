...
- [ ] 4.1 Create `communication.conversations` and `communication.messages` schema+migration with match-gated conversation creation (RC-1, RC-4)
- [ ] 4.2 Implement `ChatHub` at `/hubs/chat` with JWT auth handshake, `SendMessage` (≤2000 chars, persist-before-ack), `TypingIndicator`, `MarkRead` (RC-1, RC-2)
- [ ] 4.3 Implement `GET /conversations/{id}/messages?cursor=` with cursor-paginated ascending history, page size 50 (RC-5)
- [ ] 4.4 Implement unmatch via `POST /conversations/{id}/unmatch`: hide conversation from both, retain messages for moderation, block new messages (RC-3)
- [ ] 4.5 Add match-gated access guard: deny conversation access to non-participants and unmatched users with 403 (RC-4)

## Phase 5: Notifications (NF-1..NF-5)

- [ ] 5.1 Create `notification.notifications` and `notification.device_tokens` schema+migration with per-type opt-out flags (NF-2, NF-3, NF-4)
- [ ] 5.2 Implement `POST /notifications/register-token` with FCM/APNs token association, reassign on device-handover (NF-3)
- [ ] 5.3 Implement MediatR handlers for `MatchCreated` and `MessageSent`: create notification records + dispatch push via FCM/APNs async, respect opt-out (NF-5, NF-1)
- [ ] 5.4 Implement `NotificationHub` at `/hubs/notifications` with server→client `NewNotification` and `BadgeUpdate` (NF-2)
- [ ] 5.5 Implement `GET /notifications?cursor=`, `POST /notifications/read` (single+batch), and `PUT /notifications/opt-out` (NF-2, NF-4)

## Phase 6: Safety & Moderation (SM-1..SM-4)

- [ ] 6.1 Create `moderation.reports`, `moderation.blocks`, and `moderation.photo_reviews` schema+migration (SM-1, SM-2, SM-3)
- [ ] 6.2 Implement `POST /moderation/report` with required reason enum (Harassment, Fake Profile, Spam, Inappropriate Photos, Other), dedup same reporter+target (SM-1)
- [ ] 6.3 Implement `POST /moderation/block/{userId}` with immediate one-way block: hide from discovery, deny messages, no notification to blocked user (SM-2)
- [ ] 6.4 Implement photo moderation queue: `PendingReview` on upload confirm, admin approve→`Approved` (visible), reject→notify uploader with reason (SM-3)
- [ ] 6.5 Implement ban/unban: revoke all sessions+tokens+SignalR on ban, restore on unban, write append-only audit log entry (SM-4, AD-4)

## Phase 7: Admin Dashboard (AD-1..AD-4)

- [ ] 7.1 Create `admin.audit_log` append-only schema+migration with admin ID, action type, target user ID, timestamp, reason (AD-4)
- [ ] 7.2 Implement `GET /admin/users?q=` with email partial-match and ID exact-match, return account metadata + recent activity summary, paginated 50/page (AD-1)
- [ ] 7.3 Implement `GET /admin/reports?status=` queue sorted oldest-first, filter by Pending/Resolved/Dismissed, `POST /admin/reports/{id}/resolve` with note+audit log (AD-2)
- [ ] 7.4 Implement `POST /admin/users/{id}/ban` and `POST /admin/users/{id}/unban` with mandatory reason, audit log, immediate session+SignalR revocation (AD-3)

## Phase 8: Media Storage (MS-1..MS-4)

- [ ] 8.1 Create `media.media_files` schema+migration with blob key, status (PendingReview/Approved/Rejected), owner FK (MS-2)
- [ ] 8.2 Implement `POST /media/upload-url` generating Azure Blob SAS pre-signed PUT URL (10min expiry, 10MB max, jpeg/png/webp only), scoped to `users/{userId}/photos/{guid}.ext` (MS-1)
- [ ] 8.3 Implement `POST /media/confirm` verifying blob existence via SDK, creating `MediaFile` with `PendingReview`, triggering moderation queue (MS-2)
- [ ] 8.4 Implement CDN URL resolution for approved photos with `Cache-Control: public, max-age=86400` (MS-3)
- [ ] 8.5 Implement GDPR cascade handler: delete all user blobs from storage and `MediaFile` records on account deletion (MS-4)
