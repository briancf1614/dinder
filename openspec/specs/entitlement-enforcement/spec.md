...
The system MUST implement a MediatR `IPipelineBehavior` that reads the user's tier from JWT claims (without a DB round-trip at the gate step) and rejects commands annotated with `[RequiresTier(Tier.Plus)]` when the user's tier is insufficient.
...
#### Scenario: Tier-inadequate command rejected
